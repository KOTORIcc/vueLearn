        ...
        <ul>
          <li v-for="i in categoryList.children" :key="i.id">
            <RouterLink :to="`/category/sub/${ i.id }`">
              <img v-img-lazy="i.picture" src="" alt=""/>
              <p>{{ i.name }}</p>
            </RouterLink>
          </li>
        </ul>
...
<script>
import { useBanner } from "@/views/Layout/Category/composable/useBanner";
import {useRoute} from "vue-router";
import GoodsItem from "@/views/Layout/Home/components/GoodsItem.vue";
import {useCategory} from "@/views/Layout/Category/composable/useCategory";
...
export default {
  ...
  setup(){
    ...
    const categoryList = useCategory(route);
    const PictureList = useBanner('CategoryPageBanner')
